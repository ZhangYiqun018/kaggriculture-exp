...
当前 Champion 智能体为 **`v050_combined`**（配置为 Strawberry, No-Livestock 模式），融合了全周期草莓（Strawberry）经济学、动态雇工规模缩放（最高 10/12 手）、以及 NE 象限的条件性买地扩张（在 day >= 10, money >= 2500 时自动买入）。
...
* 💰 **v050 竞技级复合决策系统** — 协同了**CropPlan 作物排产一致性**、**NPV 收获期增量优化**、**限价控量分批销售（Holds）**、**牧场建造/动物买入/喂养/抚育/乳毛收割生命周期**、**肥料自动路由 STRAWBERRY 增产**以及 **Turn order 终局全倾销清盘**。
...
### 2. 打包、验证与可视化回放
```bash
# 4. 打包当前 champion (v050_combined) 到 dist/
...
# 6. 运行 v050 复合系统 2x2 因果消融联赛（128局）
...
基于 8 screen 种子集（双 Seat 16 场）对决 `moon` (Melon巨头) 和 `kaito_v17` (市场排队模型) 建立。

| 作物配置 | 牧场配置 | 农夫最终资金 | 对手最终资金 | 最终资金差 (Margin) | 胜/负/平 | 平均 Outcome | Wilson LB | Weeds |
|---|---|---|---|---|---|---|---|---|
| 纯作物模式 (Standard) | 无动物 | $35,081.9 | $174,698.8 | $-139,616.9 | 0/32/0 | 0.000 | 0.000 | 1.30 |
| **草莓扩张 (+STRAWBERRY)** | **无动物** | **$35,276.2** | **$174,909.1** | **$-139,632.9** | **0/32/0** | **0.000** | **0.000** | **1.30** |
| 纯作物模式 (Standard) | 奶牛 & 绵羊 (+LIVESTOCK) | $27,590.6 | $179,375.0 | $-151,784.4 | 0/32/0 | 0.000 | 0.000 | 3.00 |
| 草莓扩张 (+STRAWBERRY) | 奶牛 & 绵羊 (+LIVESTOCK) | $27,559.3 | $179,375.0 | $-151,815.7 | 0/32/0 | 0.000 | 0.000 | 4.20 |

*因果消融分析：*
- **纯草莓生产效益 (+strawberry - v031)**：带来 **+$194.3** 的现金上升（在 16 地块空间极度受限下草莓缓慢增益发挥了正效果）。
- **纯牧场动物干扰 (+livestock - v031)**：带来 **-$7,491.3** 的严重负增长。原因在于买牛（$400）买羊（$500）以及买小麦饲料等巨大的前期固定投资（超 $2000 级），在仅有 25 块农作地皮且无 ML 优化下，回本周期长于 30 天，反噬了作物的流动性资金链。
...
| **v050** (当前 Champion)| **$35,525.4** | **$35,525.4** | **$35,042.9** | **$35,027.0** | **$32,802.9** | **0 胜 80 负** |
| **净增量 (v050 vs v020)** | **+$6,513** | **+$6,513** | **+$6,238** | **+$6,015** | **+$10,943** | — |

*因果结论：* 我们的统一 Champion 智能体 `v050`（开启草莓与买地/高配Hands，关闭牧场）**实现了全线极其宏大的财务防御增量 (最高达 +$10.9k！)**。这无可辩驳地证明了：**扩充土地物理边界（NE象限买地）搭配草莓的长周期产出高加权，使得我们作物的上限取得了质的突破！**
...
```
agents/        版本化 agent（v000 - v050）与 champion 目录
...
reports/       各阶段报告 + 联赛/消融/公开前沿输出 (v031/v040/v050报告)
...
- `agents/champion` 每个版本冻结；新实验在 `agents/` 下建独立目录（如 `agents/vNNN_*`）。
