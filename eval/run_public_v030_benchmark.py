...
def write_reports(results, flat_matches):
    ...
    # 1. JSON report
    jp = reports_dir / "public_frontier_v030.json"
    jp.write_text(json.dumps(results, indent=2, default=str), encoding="utf-8")
    
    # 2. CSV match report
    cp = reports_dir / "public_frontier_v030.csv"
    ...
    md = [
        ...
        f"**Candidate:** `{CHAMPION_PATH}` (v030_market_control with market-aware allocation, NPV harvesting, chunked selling, and liquidation)",
    ...
    (reports_dir / "public_frontier_v030.md").write_text("\n".join(md), encoding="utf-8")
    print(f"Benchmark reports successfully written to: reports/public_frontier_v030.{{json,csv,md}}")
