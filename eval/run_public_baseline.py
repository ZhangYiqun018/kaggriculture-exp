...
def write_reports(results, flat_matches):
    ...
    # 1. JSON report
    jp = reports_dir / "public_frontier_v020.json"
    jp.write_text(json.dumps(results, indent=2, default=str), encoding="utf-8")
    
    # 2. CSV match report
    cp = reports_dir / "public_frontier_v020.csv"
    ...
    # 3. Markdown report
    md = [
        "# Phase 3.6: Public Opponents Frontier Baseline Report",
        f"**Candidate:** `{CHAMPION_PATH}` (repaired v020_hands with true sequential hiring)",
    ...
    (reports_dir / "public_frontier_v020.md").write_text("\n".join(md), encoding="utf-8")
    print(f"Benchmark reports successfully written to: reports/public_frontier_v020.{{json,csv,md}}")
